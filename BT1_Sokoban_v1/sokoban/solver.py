...
def breadthFirstSearch(gameState):
    ...
    startState = (beginPlayer, beginBox) # trạng thái của bản đồ
    ...
    while frontier:
        ...
        if isEndState(node[-1][-1]): # nếu node cuối cùng giống với goal thì ta lấy cộng vào temp đường đi đó
            ...
            print('BFS done') # thông báo BFS đã chạy xong
            break
        if node[-1] not in exploredSet: # nếu node không tồn tại trong tập điểm đã đi thì thêm vào
            ...
            for action in legalActions(node[-1][0], node[-1][1]): # tạo ra tập các đường đi (left,right,top,bottom)
                ...
                if isFailed(newPosBox): # nếu vị trí của hộp bị kẹt thì bỏ qua hành động
                    continue
    ...
    print(len(temp)) # in ra độ dài đường đi
    return temp
...
def uniformCostSearch(gameState):
    ...
    while frontier:
        ...
        if isEndState(node[-1][-1]):  # nếu node cuối có cost=0
            ...
            print('UCS Done') # thông báo UCS đã chạy xong
            break
        if node[-1] not in exploredSet:  # nếu node cuối chưa nằm trong tập đã đi
            exploredSet.add(node[-1])  # thêm nó vào tập đường đi
            Cost = cost(node_action[1:]) # chi phí tính từ ban đầu đến node hiện tại
            for action in legalActions(node[-1][0], node[-1][1]):
                newPosPlayer, newPosBox = updateState(node[-1][0], node[-1][1], action) # cập nhật vị trí người chơi và hộp
                if isFailed(newPosBox): # nếu vị trí của hộp bị kẹt thì bỏ qua hành động
                    continue
                frontier.push(node + [(newPosPlayer, newPosBox)],Cost) # thêm vào hàng đợi frontier chi phí đi và node đó
                actions.push(node_action + [action[-1]],Cost) # thêm action, cost action vào
    print(len(temp)) # in ra độ dài đường đi
    return temp
